...
public abstract class FourierModel extends mvc.Model {
	...
	public static double[][] data4x4() {
		double[][] sourceData = new double[][] { { 900, 901, 902, 903 }, { 910, 911, 912, 913 }, { 920, 921, 922, 923 },
				{ 930, 931, 932, 933 } };
		return sourceData;
	}
	...
	public static boolean isAllZero(double[][] aMatrix) {
		...
		for (int j = 0; j < height; j++) {
			for (int i = 0; i < height; i++) {
				if (Math.abs(aMatrix[j][i]) > FourierModel.accuracy) {
					return false;
				}
			}
		}
	...
	public static double[] normalize(double[] valueCollection) {
		...
		if (maimumValue < FourierModel.accuracy) {
			maimumValue = 1.0d;
		}
	 ...
	 * @return aValue
	 */
	public int intSwap(int swapValue,int length) {
		int halfLength = length / 2;
		if (swapValue == 509 || swapValue == 510 || swapValue == 511) {
			swapValue = 511;
		}else if(swapValue==1024){
			swapValue = 1023;
		}
		int aValue;
		if (swapValue >= halfLength) {
			aValue = swapValue - halfLength;
		} else {
			aValue = swapValue + halfLength;
		}
		return aValue;
